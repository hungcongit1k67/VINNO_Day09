...
def tool_search_kb(query: str, top_k: int = 3) -> dict:
    ...
    try:
        ...
        if not chunks:
            from workers.retrieval import _lexical_retrieve
            chunks = _lexical_retrieve(query, top_k=top_k)
...
class MCPHTTPRequestHandler(BaseHTTPRequestHandler):
    """Minimal HTTP transport for advanced MCP bonus."""

    def do_GET(self):
        if self.path == "/health":
            _json_response(self, 200, {"status": "ok", "service": "mcp_server_http"})
            return
    ...
    def do_POST(self):
        ...
        if not tool:
            _json_response(self, 400, {"error": "Missing required field: tool"})
            return

        result = dispatch_tool(tool, tool_input)
        record = {
            "tool": tool,
            "input": tool_input,
            "output": result,
    ...
    def log_message(self, format, *args):
        # Keep server output concise for lab usage.
        return


def run_http_server(host: str = "127.0.0.1", port: int = 8001) -> None:
    """Run HTTP server for Advanced Sprint 3 mode."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--serve", action="store_true", help="Run HTTP MCP server (advanced mode)")
    ...
    print(f"  notes_count: {len(perm.get('notes', []))}")

    # 5. Test invalid tool
    print("\nTest: invalid tool")
    err = dispatch_tool("nonexistent_tool", {})
    print(f"  Error returned: {bool(err.get('error'))}")

    print("\nDone: MCP server local test complete.")
    print("Tip: run `python mcp_server.py --serve` for advanced HTTP mode.")
"""
mcp_server.py — Mock MCP Server
Sprint 3: Implement ít nhất 2 MCP tools.
